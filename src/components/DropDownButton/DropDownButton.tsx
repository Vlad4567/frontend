...
export const DropDownButton = forwardRef<HTMLButtonElement, Props>(
  (
    ...
    return (
      <button
        type="button"
        className={classNames(`drop-down-button drop-down-button--${size}`, {
      ...
      >
        {active && icon && input ? (
          <>
            {children ? (
              <p className="drop-down-button__input">{children}</p>
            ) : (
              <input
                ref={inputRef}
          ...
          <>
            {children || (
              <p className="drop-down-button__text">{placeholder}</p>
            )}
          </>
        )}
        {icon && (
